...
async function createTable() {
    try {
        db.serialize(() => {
            // db.run("create table if not exists editor (id integer primary key AUTOINCREMENT, name text not null)");
            db.run("create table if not exists author (id integer primary key AUTOINCREMENT, fullname text not null)");
            db.run("create table if not exists book (id integer primary key AUTOINCREMENT, title text not null, release_date text not null, publisher text not null)");
